...
/// <summary>
/// BLE 核心管理器，提供扫描、配对、连接、读写、订阅、重连等能力�?/// </summary>
...
public sealed class BleManager : IDisposable
{
    ...
    /// <summary>
    /// 设备信息更新时触发（RSSI、连接状态、广播数据等）�?    /// </summary>
    ...
    public async Task<bool> PairAsync(string deviceId)
    {
        ...
        {
            throw new BleException($"设备不支持配�? {deviceInfo.Name} ({deviceId})");
        }
        ...
        throw new BleException($"配对失败，状�? {result.Status}");
    }
    ...
    /// <summary>
    /// 订阅指定特征�?Notify/Indicate�?    /// </summary>
    ...
    /// <summary>
    /// 手动触发重连（使用上次连接的设备 ID）�?    /// </summary>
    public async Task ReconnectAsync()
    {
        if (string.IsNullOrWhiteSpace(_connectedDeviceId))
        {
            throw new BleException("没有记录上次连接的设�?ID");
        }
    ...
    private async Task WaitForGattAsync(CancellationToken cancellationToken, int retries = 5)
    {
        ...
        throw new BleException("连接失败，无法获�?GATT 服务");
    }
    ...
    private async Task<GattCharacteristic> GetCharacteristicAsync(Guid serviceUuid, Guid characteristicUuid)
    {
        ...
        {
            throw new BleException($"未找到服�? {serviceUuid}");
        }
        ...
        {
            throw new BleException($"未找到特�? {characteristicUuid}");
        }
...
/// <summary>
/// 对外工具类入口：统一创建/获取 BLE 管理器�?/// </summary>
[SupportedOSPlatform("windows10.0.10240.0")]
public static class BleToolKit
{
    ...
    /// <summary>
    /// 获取共享管理器实例（适合单进程内全局复用）�?    /// </summary>
    public static BleManager Shared => _shared.Value;

    /// <summary>
    /// 创建独立管理器实例（适合多上下文隔离使用）�?    /// </summary>
